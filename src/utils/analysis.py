...
def prec_rec_parametrized_by_thr(
    y_true, y_pred, y_scores, label, Npoints, min_score=None, max_score=None
):
    ...
    if (min_score is None) or (max_score is None):
        predicted_in_C = y_pred == label
        min_score = 0.99 * np.amin(
            y_scores[predicted_in_C]
        )  # guarantees that all predictions are kept
        max_score = 1.01 * np.amax(
            y_scores[predicted_in_C]
...
def plot_confusion_matrix(
    ConfMat, label_strings=None, title="Confusion matrix", cmap=plt.cm.get_cmap("Blues")
):
...
class Analyzer(object):
    ...
    def show_acc_top_k_improvement(self, y_pred, y_true, k=5, inp="scores"):
        ...
        print(
            "How accuracy improves when allowing correct result being in the top 1, 2, ..., k predictions:\n"
        )
    ...
    def generate_classification_report(
        self, digits=3, number_of_thieves=2, maxcharlength=35
    ):
        """
        ...
        for i in sorted_class_indices:
            ...
            thieves_names = [
                self.existing_class_names[thief][
                    : min(maxcharlength, len(self.existing_class_names[thief]))
                ]
                for thief in thieves
            ]  # a little inefficient but inconsequential
        ...
        for v in (
            np.average(self.precision, weights=relative_freq),
    ...
    def get_avg_prec_recall(
        self, ConfMatrix, existing_class_names, excluded_classes=None
    ):
        ...
        rec_avg = np.trace(ConfMatrix[included_c, :][:, included_c]) / np.sum(
            support[included_c]
        )
    ...
    def analyze_classification(
        self, y_pred, y_true, class_names, excluded_classes=None
    ):
        """
        ...
        self.existing_class_names = [
            class_strings[ind][: min(self.maxcharlength, len(class_strings[ind]))]
            for ind in self.existing_class_ind
        ]  # a little inefficient but inconsequential
        ...
        if self.print_conf_mat:
            print_confusion_matrix(
                ConfMatrix,
                label_strings=self.existing_class_names,
        ...
        self.ConfMatrix_normalized_row = (
            ConfMatrix.astype("float") / ConfMatrix.sum(axis=1)[:, np.newaxis]
        )
        ...
        (
            ...
            y_true, y_pred, labels=self.existing_class_ind
        )
        ...
        return {
            ...
            "prec_avg": self.prec_avg,
            "rec_avg": self.rec_avg,
        }
